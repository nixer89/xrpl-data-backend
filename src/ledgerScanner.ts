    ...
    async scheduleLoadingIssuerData(): Promise<void> {
     ...
     public async readLedgerData(ledgerIndex:number, marker:unknown, oldMarker:unknown, retryCounter:number): Promise<boolean> {
        ...
        try { 
          ...
          let bufferBinary = Buffer.from(JSON.stringify(messageBinary),'utf8');
          console.log("Binary length: " + bufferBinary.byteLength);
          ...
          if(messageBinary && messageBinary && messageBinary.state && messageBinary.ledger_index) {
            ...
            let bufferJson = Buffer.from(JSON.stringify(messageJson),'utf8');
            console.log("JSON length: " + bufferJson.byteLength);
